...
#
# Dispatch the single vgi-asn1 image into one of its transports:
...
#
# The worker is stateless (pure asn1-core compute over the input blob), so there
# is no /data to create and no state env to wire — each mode just exec's the
# binary.
set -e
...
case "${1:-http}" in
  http)
    ...
    exec asn1-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec asn1-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec asn1-worker "$@"
