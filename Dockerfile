...
#
# Single image that serves every network transport of the `asn1` VGI worker:
...
#
# Like vgi-units this worker is STATELESS: all correctness lives in the pure
# `asn1-core` engine compiled into the binary (no scan state, no secrets, no
# network, no per-attach storage), so there is no /data volume, no state
...
# runtime ships. `vgi`/`arrow` resolve from crates.io — no git patch deps.
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked --bin asn1-worker \
    && cp target/release/asn1-worker /asn1-worker
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-asn1
...
LABEL org.opencontainers.image.title="vgi-asn1" \
      org.opencontainers.image.description="Decode ASN.1 BER/CER/DER and named security/telecom modules (SNMP, Kerberos, LDAP, CMS/PKCS, OCSP) as a VGI worker for DuckDB/SQL (stdio + HTTP + TCP)" \
...
ENV PORT=8000 \
    ...
    VGI_ASN1_GIT_COMMIT=${GIT_COMMIT}
...
COPY --from=build --chmod=0755 /asn1-worker /usr/local/bin/asn1-worker
